...
class StompBrokerForgeryIntegrationTest {
    ...
    void forgedRoomEventIsRejectedAndNotDeliveredToAnotherMember() throws Exception {
        ...
        commandService.changeReady(roomId, owner.id(), true);
        String firstEvent = receivedRoomEvents.poll(5, TimeUnit.SECONDS);
        String secondEvent = receivedRoomEvents.poll(5, TimeUnit.SECONDS);

        assertThat(firstEvent)
            .as("첫 번째 Room 이벤트가 도착해야 한다.")
            .isNotNull();

        assertThat(secondEvent)
            .as("두 번째 Room 이벤트가 도착해야 한다.")
            .isNotNull();

        List<String> readyEvents = List.of(firstEvent, secondEvent);

        assertThat(readyEvents)
            .anySatisfy(event ->
                assertThat(event).contains("ROOM_READY_CHANGED")
            );

        assertThat(readyEvents)
            .anySatisfy(event ->
                assertThat(event).contains("ROOM_STATE_UPDATED")
            );
        StompSession attackerSession = connect(attacker, errorHandler(attackerErrors));
