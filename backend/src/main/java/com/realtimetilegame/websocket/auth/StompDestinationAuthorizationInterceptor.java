...
public class StompDestinationAuthorizationInterceptor implements ChannelInterceptor {
    ...
    private static final Pattern GAME_SEND = Pattern.compile(
        "^/app/games/(\\d+)/turn/(draw|pass|commit|preview(?:/cancel)?)$"
    );
    private static final Pattern GAME_EXIT_SEND = Pattern.compile("^/app/games/(\\d+)/exit$");
    ...
    private void authorizeSend(String destination, Principal principal) {
        ...
        Long gameId = matchedPositiveId(GAME_SEND, destination);
        if (gameId != null) {
            StompPrincipal authenticated = requireActivePrincipal(principal);
        ...
        Long exitGameId = matchedPositiveId(GAME_EXIT_SEND, destination);
        if (exitGameId != null) {
            StompPrincipal authenticated = requireActivePrincipal(principal);
            requireHistoricalGameMembership(exitGameId, authenticated.userId());
            return;
        }
    ...
    private void requireHistoricalGameMembership(long gameId, long userId) {
