...
public enum ErrorCode {
    ...
    INVALID_GAME_ACTION_ID(HttpStatus.BAD_REQUEST, "유효한 게임 actionId가 필요합니다."),
    INVALID_GAME_VERSION(HttpStatus.BAD_REQUEST, "유효한 gameVersion이 필요합니다."),
    GAME_NOT_IN_PROGRESS(HttpStatus.CONFLICT, "진행 중인 게임에서만 행동할 수 있습니다."),
    ROOM_GAME_MISMATCH(HttpStatus.CONFLICT, "방과 게임 식별자가 일치하지 않습니다."),
    NOT_CURRENT_TURN(HttpStatus.CONFLICT, "현재 턴 사용자만 행동할 수 있습니다."),
    STALE_GAME_VERSION(HttpStatus.CONFLICT, "게임 상태가 변경되었습니다. 최신 상태를 다시 불러와주세요."),
    DRAW_POOL_EMPTY(HttpStatus.CONFLICT, "Pool이 비어 있어 타일을 뽑을 수 없습니다."),
    PASS_NOT_ALLOWED(HttpStatus.CONFLICT, "Pool에 타일이 남아 있어 PASS할 수 없습니다."),
    EMPTY_MELD_SUBMISSION(HttpStatus.BAD_REQUEST, "제출할 Meld가 필요합니다."),
    DUPLICATE_MELD_ID(HttpStatus.BAD_REQUEST, "같은 Meld ID를 중복 제출할 수 없습니다."),
    DUPLICATE_TILE_IN_TURN(HttpStatus.BAD_REQUEST, "같은 타일을 한 턴에 중복 제출할 수 없습니다."),
    TILE_NOT_IN_RACK(HttpStatus.CONFLICT, "자신의 Rack에 있는 타일만 제출할 수 있습니다."),
    INVALID_MELD(HttpStatus.CONFLICT, "유효하지 않은 RUN 또는 GROUP이 포함되어 있습니다."),
    INITIAL_MELD_SCORE_TOO_LOW(HttpStatus.CONFLICT, "첫 등록 합계는 30점 이상이어야 합니다."),
    INVALID_TABLE_LAYOUT(HttpStatus.CONFLICT, "제출한 Table 배치를 확정할 수 없습니다."),
    INVALID_COMMIT_PAYLOAD(HttpStatus.BAD_REQUEST, "턴 확정 요청 형식을 확인해주세요."),
