...
public class GameStartService {
    ...
    public GameStartResult startGame(long roomId, long requesterUserId) {
        ...
        Game game = gameRepository.saveAndFlush(Game.startClassic(
            room,
            firstRoomPlayer.user(),
            firstRoomPlayer.seatOrder(),
        ...
        GamePublicState publicState = stateAssembler.publicState(game, gamePlayers, gameTiles, List.of());
        Map<Long, GamePrivateState> privateStates = stateAssembler.privateStates(game, gamePlayers, gameTiles, List.of());
    ...
    private void verifyPersistedState(Game game, List<GamePlayer> players, List<GameTile> tiles) {
        ...
        if (gameRepository.countByRoomId(game.room().id()) != 1L
            ...
            || gameMeldRepository.countByGameId(game.id()) != 0L
