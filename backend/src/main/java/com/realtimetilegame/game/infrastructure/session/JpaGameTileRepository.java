...
public class JpaGameTileRepository implements GameTileRepository {
    ...
    @Override public Optional<GameTile> findFirstPoolTileForUpdate(long gameId) {
        return repository.findFirstByGameIdAndLocationOrderByPositionOrderAsc(gameId, GameTileLocation.POOL);
    }
    @Override public int findNextRackPosition(long gamePlayerId) {
        return Math.addExact(repository.findMaxRackPosition(gamePlayerId), 1);
    }
