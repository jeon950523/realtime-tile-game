...
@Component
public class ActionReplayStore {
    ...
    public ActionReplayStore(Clock clock) { this.clock = clock; }

    public Result execute(long userId, String actionId, Supplier<RoomCommandReply> action) {
        cleanup();
        String key = userId + ":" + actionId;
        ...
        try {
            RoomCommandReply reply = action.get();
    ...
    private record Entry(CompletableFuture<RoomCommandReply> future, Instant createdAt) {}
    public record Result(RoomCommandReply reply, boolean duplicate) {}
}
