...
const routeRoomId = computed(() => Number(route.params.roomId))
const launchingGame = ref(false)

const room = computed(() => roomStore.activeRoom?.roomId === routeRoomId.value ? roomStore.activeRoom : null)
...
watch(
  () => routeRoomId.value,
  async (nextRoomId, _previousRoomId, onCleanup) => {
    if (route.name !== 'waiting-room') return
    ...
    try {
      await roomStore.loadRoomDetail(nextRoomId)
      if (cancelled || routeRoomId.value !== nextRoomId) return
      await roomStore.connectRoom(nextRoomId)
    } catch {
      if (cancelled || routeRoomId.value !== nextRoomId) return
      roomStore.clearTerminatedRoom(nextRoomId)
      await router.replace('/lobby')
    }
  },
  { immediate: true },
...
async function leave(): Promise<void> {
  try {
    const left = await roomStore.leaveRoom(routeRoomId.value)
    if (left) await router.replace('/lobby')
  } catch {
    // Store exposes safe error and keeps the user in the room.
  }
}

async function logout(): Promise<void> {
  try {
    await roomStore.leaveRoom(routeRoomId.value)
