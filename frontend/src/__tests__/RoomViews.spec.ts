...
describe('room views', () => {
  ...
  it('reloads room detail and realtime subscription when only the route room id changes', async () => {
    const room11: RoomDetail = {
      ...
      ownerNickname: 'guest',
      participants: [
        { userId: 2, nickname: 'guest', avatarType: 'DEFAULT_02', seatOrder: 1, readyStatus: 'NOT_READY', owner: true },
      ],
      currentPlayers: 1,
      startBlockReason: 'ROOM_MIN_PLAYERS_NOT_MET',
    }
    vi.mocked(roomApi.getRoomDetail).mockImplementation(async (roomId) => roomId === 11 ? room11 : detail)
    const router = await routerAt('/rooms/10')
    const wrapper = mount(WaitingRoomView, { global: { plugins: [router] } })
    await flushPromises()

    expect(wrapper.text()).toContain('초보방')
    await router.push('/rooms/11')
    await flushPromises()

    expect(roomApi.getRoomDetail).toHaveBeenCalledWith(11)
    expect(realtime.connectRoom).toHaveBeenLastCalledWith(11)
    expect(wrapper.text()).toContain('새 방')
    expect(wrapper.text()).not.toContain('초보방')
  })

  it('passes the current route room id to leave even if the store context became stale', async () => {
    const room11: RoomDetail = { ...structuredClone(detail), roomId: 11, roomName: '새 방' }
    vi.mocked(roomApi.getRoomDetail).mockResolvedValue(room11)
    vi.mocked(roomApi.leaveRoom).mockResolvedValue(undefined)
    const router = await routerAt('/rooms/11')
    const wrapper = mount(WaitingRoomView, { global: { plugins: [router] } })
    await flushPromises()
    const store = useRoomStore()
    store.activeRoom = room11
  ...
  it('does not navigate when the store did not execute the leave request', async () => {
    const router = await routerAt('/rooms/10')
    const wrapper = mount(WaitingRoomView, { global: { plugins: [router] } })
    await flushPromises()
    const store = useRoomStore()
