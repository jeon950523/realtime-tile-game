...
| 영역 | 사용자 환경 최종 결과 |
|---|---|
| Backend `./mvnw clean test` | **258개 통과**, Failures 0, Errors 0, Skipped 0, BUILD SUCCESS, Java 17 |
| Frontend Vitest | **77개 통과** |
| TypeScript | 통과 |
| Production Build | 통과 |
| MySQL | V1·V2·V3 정상 |
| Chrome 2계정 | 같은 Game 진입, Rack 14개씩, Pool 78, 상대 Rack 상세 비공개, WebSocket 정상 |

## 과거 검수 환경 이력

최초 문서 생성 환경에서는 Maven 저장소 DNS 문제로 Backend 실행이 중단됐고 Frontend만 77개가 통과했다. 이후 사용자 환경에서 Backend 258개를 포함한 전체 검증을 완료했으므로 위 표가 최신 기준이다.
...
Backend 258개와 Frontend 77개를 사용자 환경에서 최종 확인했다. 이후 변경 시 동일 명령으로 회귀 검증한다.
