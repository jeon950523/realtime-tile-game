...
# 36. Phase 7 영속 Table 모델

현재 공개 Snapshot의 Table은 typed 구조다.

```text
GamePublicState.tableMelds: GameTableMeldView[]
...
권위 타일 분할은 항상 다음을 만족한다.

```text
106 Tiles = RACK ∪ POOL ∪ TABLE
각 집합은 서로소
TABLE Tile은 정확히 하나의 GameMeld에 연결
RACK Tile만 owner_game_player_id 보유
```

TurnDraft는 서버 Snapshot이 아니라 클라이언트 로컬 후보 상태다. COMMIT 시 `GameTurnStateFactory`가 영속 상태를 기존 Rule Engine의 `TurnState`로 변환하고, 검증된 결과만 `GameMeld`와 TABLE Tile로 반영한다.

---

# 37. Phase 7 Second Review Working Table과 Reconciliation

클라이언트는 내 턴의 서버 `tableMelds`를 baseline으로 깊은 복사하고 전체 Working Table을 편집한다. 첫 등록 전에는 baseline이 순서와 내용까지 동일한 prefix여야 하며, 완료 후에는 baseline TABLE 타일을 정확히 한 번 보존하는 범위에서 Meld 재구성이 가능하다.

서버 Commit은 전체 Candidate를 `TurnCommitValidator`, `InitialMeldValidator`, `TableRearrangementValidator`로 검증한다. 성공 시 기존 Meld/TABLE positions를 임시 안전 영역으로 옮긴 뒤 최종 Meld와 타일 positions를 재연결하고 orphan Meld를 제거한다. Game lock, Rack 감소, Table 치환, initial flag, 다음 턴, gameVersion 증가는 한 트랜잭션이다.
